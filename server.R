...
server <- function(input, output, session) {
  ...
  observe({
    ...
    if (!is.null(account_type)) {
      refreshTransactionTable(account_type, cache_env, con)  # 优化后的表格刷新
      resetToCreateMode()
  ...
  observeEvent(input$record_transaction, {
    ...
    if (is_update_mode()) {
      ...
      tryCatch({
        ...
        # 自动更新账户余额和表格
        updateAccountOverview(output, con)
        refreshTransactionTable(account_type, cache_env, con)
      }, error = function(e) {
        showNotification(paste("更新失败：", e$message), type = "error")
      })
    } else {
  ...
  observeEvent(input$record_transfer, {
    ...
    tryCatch({
      ...
      # 自动刷新表格
      refreshTransactionTable(input$from_account, cache_env, con)
      refreshTransactionTable(input$to_account, cache_env, con)
